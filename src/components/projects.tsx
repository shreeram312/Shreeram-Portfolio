  ...
  return (
    <div className="bg-secondary cursor-pointer rounded-xl w-80 h-96 flex flex-col justify-between border-transparent border-2 hover:border-blue-500 transition-all transform hover:scale-105 hover:shadow-lg">
      <div className="px-6 pt-6 flex-grow">
        <h1 className="text-xl font-serif text-white font-serif">{title}</h1>
        <h2 className="text-sm pb-2 text-white font-serif">{subtitle}</h2>
        ...
        <div className="flex flex-wrap gap-1 font-serif ">
          {stack.map((tech, index) => (
            <Badge variant="blue" key={index} className="rounded-3xl px-3 py-1">
              {tech}
            </Badge>
          ))}
        </div>
